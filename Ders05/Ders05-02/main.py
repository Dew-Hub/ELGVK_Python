...
def paraYatirRootEkrani():
    paraYatirRoot = tk.Toplevel(userInfoRoot)
...
def paraCekRootEkrani():
    global paraCekRoot
    paraCekRoot = tk.Toplevel(userInfoRoot)
...
def dovizAlRootEkrani():
    global dovizAlRoot
    global alinacakMiktar

    dovizAlRoot = tk.Toplevel(userInfoRoot)
    dovizAlRoot.title("Para Çekme")
    dovizAlRoot.geometry("600x200")
    dovizData = guncelDovizKurCek()

    dovizLabel = tk.Label(dovizAlRoot, text=f"USD Güncel Kur: {dovizData["USD"]}\nEUR Güncel kur: {dovizData["EUR"]}", font=("Ariel", 20)).pack()
    ttk.Combobox(dovizAlRoot, values=["USD","EUR"], state="readonlt", width=15).pack()
    tk.Label(dovizAlRoot, text="İşlem Türü").pack()

    islemVarAl = tk.StringVar(dovizAlRoot, "İşlem")
    tk.Radiobutton(dovizAlRoot,variable=islemVarAl, text="Döviz Al", value="Al").pack()
    tk.Radiobutton(dovizAlRoot,variable=islemVarAl, text="Döviz Sat", value="Sat").pack()

    alinacakMiktar = tk.Entry(dovizLabel)
    alinacakMiktar.pack()
    ...
    def dovizIslemYap():
        ...
        '''
        miktar = float(alinacakMiktar.get())
        mevcutMiktar = float(user[tcKimlik]["bakiye"])
        ...
        try:
            ...
            messagebox.showerror("hata","dosyaya yazarken hata oluştu")
        '''
    tk.Button(dovizAlRoot, text="İşlem yap", command=dovizIslemYap).pack()
...
def atmEkrani(userTcKimlikNo):
    ...
    dovizAlButton = tk.Button(userInfoRoot,text="Döviz Al", command=dovizAlRootEkrani)
    dovizAlButton.pack()
